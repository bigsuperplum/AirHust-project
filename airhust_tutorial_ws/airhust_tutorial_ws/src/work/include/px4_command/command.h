  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::px4_command::command_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.pos_sp.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.pos_sp[i]);
    }
    if (v.pos_sp.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.vel_sp.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.vel_sp[i]);
    }
    if (v.vel_sp.empty() || true)
      s << "]";
    if (true || !indent.empty())
