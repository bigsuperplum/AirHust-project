...
using namespace std;
extern ros::Publisher mavros_setpoint_pos_pub;
extern ros::Rate* global_rate; // 用指针避免构造问题
...
*/
static bool safe_land_with_disarm()
{
    static bool landing_phase = 0; // 0: 降落, 1: 停机
    static ros::Time phase_time;
    static float target_altitude = -0.15;
    static float land_position_x = 0;
    static float land_position_y = 0;
    
    if (landing_phase == 0)
    {
        // 记录降落位置
        land_position_x = local_pos.pose.pose.position.x;
        land_position_y = local_pos.pose.pose.position.y;
        
        // 设置降落目标
        setpoint_raw.type_mask = 0b111111111000;
        setpoint_raw.coordinate_frame = 1;
        setpoint_raw.position.x = land_position_x;
        setpoint_raw.position.y = land_position_y;
        setpoint_raw.position.z = target_altitude;
        
        // 检查是否到达目标高度
        if (local_pos.pose.pose.position.z <= target_altitude + 0.1)
        {
            landing_phase = 1;
            phase_time = ros::Time::now();
            ROS_INFO("Landed on ground, preparing to disarm...");
        }
    }
    else if (landing_phase == 1)
    {
        ...
        // 等待一小段时间后停机
        if (ros::Time::now() - phase_time > ros::Duration(1.0))
        {
            // 尝试停机
            static ros::ServiceClient disarm_client = ros::NodeHandle().serviceClient<mavros_msgs::CommandBool>("mavros/cmd/arming");
            
            mavros_msgs::CommandBool disarm_cmd;
            disarm_cmd.request.value = false;
            
            if (disarm_client.call(disarm_cmd) && disarm_cmd.response.success)
            {
                ROS_INFO("Vehicle disarmed successfully. Motors stopped.");
                landing_phase = 0; // 重置状态
                return true;
            }
            else
            {
                ROS_WARN("Failed to disarm vehicle, retrying...");
                phase_time = ros::Time::now(); // 重置时间以重试
            }
        }
    }
